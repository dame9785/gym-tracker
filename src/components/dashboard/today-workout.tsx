    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
      ...
      {hasWorkout ? (
        <>
          ...
          <p className="mt-2 text-zinc-400">{workout.status === 'COMPLETED' ? 'Workout completed today.' : 'Ready to train today.'}</p>

          {workout?.status === 'ACTIVE' && (
            <button onClick={handleStartWorkout} className="mt-6 rounded-lg bg-orange-500 px-5 py-3 font-semibold text-white transition hover:bg-orange-600">
              Start workout
            </button>
          )}

          {workout?.status === 'COMPLETED' && (
            <div className="mt-6 inline-flex items-center rounded-lg bg-green-600 px-5 py-3 font-semibold text-white">✅ Workout Completed</div>
          )}
        </>
      ) : (
        <>
