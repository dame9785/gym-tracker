...
export default function Modal({ isOpen, onClose, title, children }: ModalProps) {
  ...
  return (
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-5xl rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl">
        ...
        <div className="custom-scrollbar max-h-[80vh] overflow-y-auto">{children}</div>
      </div>
    </div>
  );
}
