...
export default function WeightProgress({ weightData }: Props) {
  ...
  return (
    <div className="mb-10 grid min-w-0 grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
      {/* Start weight */}
      <div className="group relative min-w-0 overflow-hidden rounded-2xl border border-zinc-800 bg-linear-to-br from-zinc-900 to-zinc-950 p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-orange-500/10 text-orange-400 transition-transform duration-300 group-hover:scale-110">
        ...
        <div className="mt-1 flex items-baseline gap-2">
          <h2 className="truncate text-3xl font-bold tracking-tight text-white">{start.toFixed(1)}</h2>
      ...
      <div className="group relative min-w-0 overflow-hidden rounded-2xl border border-zinc-800 bg-linear-to-br from-zinc-900 to-zinc-950 p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-medium uppercase tracking-wider text-zinc-600">Nu</span>
        </div>

        <p className="mt-6 text-sm font-medium text-zinc-400">Nuvarande vikt</p>

        <div className="mt-1 flex items-baseline gap-2">
          <h2 className="truncate text-3xl font-bold tracking-tight text-white">{current.toFixed(1)}</h2>
      ...
      {/* Goal weight */}
      <div className="group relative min-w-0 overflow-hidden rounded-2xl border border-zinc-800 bg-linear-to-br from-zinc-900 to-zinc-950 p-6">
        <div className="absolute left-0 top-0 h-px w-full bg-linear-to-r from-purple-500/70 via-purple-500/20 to-transparent" />

        <div className="flex items-center justify-between">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-purple-500/10 text-purple-400 transition-transform duration-300 group-hover:scale-110">
            <Target className="h-5 w-5" />
          </div>

          <span className="text-xs font-medium uppercase tracking-wider text-zinc-600">Mål</span>
        </div>

        <p className="mt-6 text-sm font-medium text-zinc-400">Målvikt</p>

        <div className="mt-1 flex items-baseline gap-2">
          <h2 className="truncate text-3xl font-bold tracking-tight text-white">{goal.toFixed(1)}</h2>
      ...
      {/* Weight change */}
      <div
        className={`group relative min-w-0 overflow-hidden rounded-2xl border bg-linear-to-br from-zinc-900 to-zinc-950 p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
      ...
      >
        <div className={`absolute left-0 top-0 h-px w-full ${isWeightGain ? 'bg-linear-to-r from-green-500/70 via-green-500/20 to-transparent' : 'bg-linear-to-r from-red-500/70 via-red-500/20 to-transparent'}`} />

        <div className="flex items-center justify-between">
          <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${isWeightGain ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'} transition-transform duration-300 group-hover:scale-110`}>
