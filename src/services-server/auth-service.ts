...
export class AuthService {
  ...
  async register(dto: RegisterUserDto): Promise<AuthApiResponse> {
    ...
    const fieldErrors: Partial<
      Record<keyof RegisterUserDto, string>
    > = {};
    ...
    try {
      ...
      const userData = UserMapper.userDtoToDbModel(
        dto,
        passwordHash,
      );
      ...
      if (
        ...
        const fieldErrors =
          this.getUniqueConstraintFieldErrors<RegisterUserDto>(error);
  ...
  async getUserById(
    id: number,
  ): Promise<UserSettingsViewModel | null> {
    const user = await this.userRepository.findById(id);
    if (!user) {
      return null;
    }
    return UserMapper.userModelToViewModel(user);
  }
  // --------------------------------------------------
  // Update User
  // --------------------------------------------------
  async updateUser(
    ...
    const fieldErrors: Partial<
      Record<keyof UpdateUserDto, string>
    > = {};
    ...
    try {
      ...
      if (
        ...
        const fieldErrors =
          this.getUniqueConstraintFieldErrors<UpdateUserDto>(error);
        const errors = Object.values(fieldErrors);
        return {
          success: false,
          message: 'Valideringen misslyckades.',
  ...
  // Private Helpers
  // --------------------------------------------------
  private getUniqueConstraintFieldErrors<
    T extends RegisterUserDto | UpdateUserDto,
  >(
